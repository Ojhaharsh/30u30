...
Build the complete Transformer encoder-decoder architecture!

This is the culmination of all previous exercises:
- Scaled dot-product attention (Exercise 1)
- Multi-head attention (Exercise 2)
- Positional encoding (Exercise 3)
- Encoder block (Exercise 4)
...
class DecoderBlock:
    ...
    TODO: Reference implementation provided - understand it!
    """
    ...
    def forward(self, x: np.ndarray, encoder_output: np.ndarray,
        ...
        Args:
            ...
            encoder_output: From encoder (batch, seq_src, d_model)
...
class Transformer:
    ...
    TODO: Implement the forward method using all the components.
    """
    ...
    def forward(self, src: np.ndarray, tgt: np.ndarray) -> np.ndarray:
        ...
        TODO: Implement using encode() and decode()!
        """
        # ==========================================
        ...
        raise NotImplementedError("Implement full Transformer forward pass!")
...
def test_causal_mask():
    ...
    assert np.array_equal(mask[0, 0], expected), "Causal mask incorrect"
...
def test_encoder():
    ...
    assert encoder_output.shape == (2, 8, 32), f"Shape: {encoder_output.shape}"
...
def test_decoder():
    ...
    assert decoder_output.shape == (2, 6, 32), f"Shape: {decoder_output.shape}"
...
def test_full_forward():
    ...
    assert logits.shape == (2, 6, 100), f"Shape: {logits.shape}"
...
def test_prediction():
    ...
    tgt = np.array([[1]])  # Start token
    
    logits = transformer.forward(src, tgt)
    prediction = logits.argmax(axis=-1)
    
    assert prediction.shape == (1, 1), f"Shape: {prediction.shape}"
    print(f"[PASS] Prediction test passed! (predicted: {prediction[0, 0]})")
...
def test_copy_task():
    ...
    src = np.array([[1, 2, 3, 4, 5]])
    tgt_input = np.array([[0, 1, 2, 3, 4]])  # Shifted right
    
    logits = transformer.forward(src, tgt_input)
    
    # Check shapes
    assert logits.shape == (1, 5, 10), f"Shape: {logits.shape}"
...
def run_all_tests():
    ...
    try:
        ...
        print("=" * 50)
        print("\nCongratulations! You've built a complete Transformer!")
